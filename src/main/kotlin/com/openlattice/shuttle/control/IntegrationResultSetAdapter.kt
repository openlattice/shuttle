...
class IntegrationResultSetAdapter {
    ...
    companion object {
        ...
        fun integration(rs: ResultSet): Integration {
            ...
            val contacts = PostgresArrays.getTextArray(rs, CONTACTS_FIELD).toSet()
            val recurring = rs.getBoolean(RECURRING.name)
            return Integration(name, sql, src, srcPkeyCols, environment, defaultStorage, s3bucket, flight, contacts, recurring, 0L, 0L)
        }
